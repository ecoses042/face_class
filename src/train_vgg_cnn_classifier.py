...
VGG-like CNN으로 AIHub 이미지에서 나이 3-class 분류를 학습한다.
클래스: young (<20) / adult (20-60) / senior (60+)

실행:
    python src/train_vgg_cnn_classifier.py [--img-size 128] [--epochs 30] [--lr 1e-3] [--batch-size 32]
"""
...
RESULT_DIR  = RESULTS_DIR / "cnn_vgg_classifier"
TITLE       = "VGG CNN Classifier"
...
def main():
    ...
    model = build_cnn_model(img_size=args.img_size, num_classes=3)
    model.summary()
    ...
    axes[1].legend(); axes[1].set_title(f"{TITLE} — Accuracy")
    fig.tight_layout()
