"""
train_cnn_aihub.py
VGG-like CNN (~33.6M) 으로 AIHub 이미지에서 나이 회귀를 학습한다.
FCN 과 동일한 split_meta.csv 분할을 사용한다.

실행:
    python src/train_cnn_aihub.py [--img-size 128] [--epochs 30] [--lr 1e-3] [--batch-size 32]
"""
...
def main():
    ...
    model = build_cnn_regressor(img_size=args.img_size)
    model.summary()

    run_training(
        model, train_ds, valid_ds, test_ds, test_df,
        result_dir=RESULTS_DIR / f"cnn_aihub_{tag}",
        title=f"CNN-AIHub (VGG-like, 33.6M, {tag})",
        epochs=args.epochs,
